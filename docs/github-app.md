...
The future commit path must not guess a bot email from the App slug. The current policy grants only `contents:read` and `pull_requests:read` on `baziyer/dark-factory`; it cannot create a branch, commit, or PR, and it cannot reach an out-of-scope disposable repository. Therefore the read-only path makes no verification claim and emits an explicitly non-executable `github_app_identity_verification` temporary-authority proposal template with `authority_ready: false`. The template includes immutable slots for proposal/idempotency identity, exact repository and branch/PR targets, approver, expiry, permission/event/identity revisions, allowed and forbidden operations, and teardown identities. Machine-checked blockers reject it until every slot is filled; it is durable audit state only, not registration, permission expansion, credential minting, or provider input.
...
- [ ] If identity verification is needed, keep the typed temporary-authority proposal template `authority_ready: false` until an operator separately supplies and approves every immutable target, approver, expiry, revision, operation-bound, idempotency, and teardown identity; do not treat the current read-only App scope as able to push.
