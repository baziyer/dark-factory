...
1. `./scripts/new-worktree.sh <slug>` — one worktree per task, never work
   directly on `main`. The script prints the new absolute path. It derives the
...
6. Remove the worktree once merged (`git worktree remove <absolute-path>`, using
   the path printed when it was created).
