...
1. **Build and publish**: pushing a semver tag (`git tag v0.2.1 && git push
   origin v0.2.1`, on a commit whose `Cargo.toml` workspace version is
   `0.2.1` — the workflow refuses a mismatch) runs
   ...
   retried. A tag with a pre-release suffix (`v0.2.1-rc.1`) is published as
