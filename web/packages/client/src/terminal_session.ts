...
class TerminalHandleImpl implements InternalTerminalHandle {
  ...
  receiveError(id: string, error: SessionErrorLike): boolean {
    ...
    if (operation.kind === "renew" || operation.kind === "detach") { this.#fatal(error); return true; }
    this.#operation = undefined;
    if (operation.kind === "acquire") { this.#cancelTimer(); this.#renewDue = false; this.#lease = undefined; }
    // A refused release still ends this client's authority: whatever the
    // daemon's reason (the run ended, the lease lapsed or moved on), it no
    // longer honours this generation, so it is fenced here and observation
    // continues. A detach in progress carries on without the lease.
    if (operation.kind === "release") this.#clearAuthority(operation.generation);
  ...
  #advanceDetach(): void {
    ...
    try {
      // A release refused by the daemon is handled where it is received and
      // the detach goes on; every other way it fails has already been fatal.
      if (this.#lease !== undefined) void this.#beginRelease(true).catch(() => undefined);
