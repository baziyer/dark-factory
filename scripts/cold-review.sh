...
# request is no longer at the stated head, 2 for bad arguments. The session's
# final message lands in review-PR-HEAD8.log in the current directory. Whether
# a verdict was really recorded is the merge queue's review check to decide,
# not this script's.
#
# The session runs from the scratch directory, not from the checkout: a
# CLAUDE.md, AGENTS.md or .claude directory in the change under review must
# not become the reviewer's own instructions.
set -eu
if [ "$#" -lt 5 ]; then
...
prompt="You are an independent, adversarial cold reviewer for pull request #$pr in $repository at exact head commit $head, whose base is $base. You have not seen this work before; the author is not present. Verify, do not trust: read the pull request body at $work/body.md, read the diff with 'git -C $work/repo diff $base $head' (every git command takes -C $work/repo, a checkout at that head; read its files by absolute path), read the surrounding source there, and look for real defects: wrong behaviour, missing or declaration-restating tests, unhandled edge cases, races, security or trust-boundary gaps, claims in the body the diff does not support, owner identity leaks (emails, org names, /Users/<name> paths) in code, tests, fixtures, commit or pull request text, and violations of AGENTS.md (ponytail ladder: unrequested abstractions, needless code, net production delta not stated). Focus areas: ${focus:-none given}. Then record your verdict through the Maintainer App: call the maintainer MCP tool submit_pull_request_review for repository $repository, pull request $pr, head_sha $head, with a fresh operation_id from uuidgen, event ALLOW only if you found no defect that must change before merge, otherwise REQUEST_CHANGES, and a body listing every finding with file:line and why it matters. Deferred notes that need no change may accompany an ALLOW. Do not edit files. Finish with the findings in plain text and, as the very last line of your reply, exactly one of: VERDICT: ALLOW or VERDICT: REQUEST_CHANGES"
cd "$work"
claude -p "$prompt" --model opus \
    --strict-mcp-config --mcp-config "{\"mcpServers\":{\"maintainer\":{\"command\":\"$bridge\"}}}" \
    --allowedTools "mcp__maintainer__submit_pull_request_review,Bash(git -C $work/repo:*),Bash(uuidgen),Read,Grep,Glob" \
    > "$out" 2>&1 || true
tail -60 "$out"
