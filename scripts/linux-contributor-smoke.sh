...
# Crash during the first immutable test launch, mutate the retained writable
# Change, and prove recovery executes the already selected snapshot rather than
# taking a second source view.
attempt=0
while ! test -s "$scratch/verifier-launches"; do
    attempt=$((attempt + 1))
    test "$attempt" -lt 300 || {
        "$factoryctl" status >&2 || true
        cat "$scratch/factoryd.log" >&2
        echo "Rust verifier never launched its prepared test" >&2
        exit 1
    }
    sleep 0.1
done
...
verifier_launch_count=$(wc -l <"$scratch/verifier-launches" | tr -d '[:space:]')
case "$verifier_launch_count" in
    1|2) ;;
    *)
    cat "$scratch/verifier-launches" >&2 2>/dev/null || true
    echo "Rust verifier restart neither consumed the exact result nor performed one bounded replacement launch" >&2
    exit 1
    ;;
esac
run_list
...
"$factoryctl" storage status --json >"$scratch/storage.json"
if ! grep -Fq '"complete":true' "$scratch/storage.json" \
    || ! grep -Fq '"cache_count":1' "$scratch/storage.json" \
    || grep -Fq '"cache_count_over_limit":true' "$scratch/storage.json" \
    || grep -Fq '"cache_bytes_over_limit":true' "$scratch/storage.json"; then
    cat "$scratch/storage.json" >&2
    echo "Rust completion cache was not bounded and fully measured" >&2
    exit 1
fi
test ! -d "$home/artifacts/tmp" || test -z "$(find "$home/artifacts/tmp" -mindepth 1 -print -quit)" || {
    find "$home/artifacts/tmp" -mindepth 1 -maxdepth 3 -print >&2
    echo "terminal task retained verifier staging" >&2
    exit 1
}
...
echo "Linux source smoke passed: exact .git-free Change, provider and verifier crash/restart recovery, completion-owned Rust verification, immutable staged launch, bounded cache reporting, explicit removal, and resource teardown"
